...
const PlaceOrder = () => {
	...
	const { loggedInUserId, accessToken, isAccessTokenValid, logout } = useContext(AuthContext);
	...
	const initPageData = useCallback((data = json, redirect = navigate, showMessage = broadcastAlert) => {
		if (data === null) {
			showMessage("Invalid access. Redirecting to home...", "warning");
			redirect("/home");
		}
	}, [json, navigate, broadcastAlert]);
	useEffect(() => {
		initPageData();
	}, [initPageData]);
	...
	let confirmAndPlaceOrder = () => {
		if (isAccessTokenValid()) {
			createOrder(orderDetails, accessToken).then(() => {
				broadcastAlert("Order placed successfully!", "success");
				navigate("/home");
			}).catch((json) => {

...
const ProductListing = ({ productList, sortBy, category, reFetchAllData }) => {

	const [deleteModal, setDeleteModal] = useState(false);
	const [product, setProduct] = useState(null);
	const { AuthContext } = useAuth();
	const { hasRole, accessToken, isAccessTokenValid, logout } = useContext(AuthContext);
	...
	const proceedDelete = () => {
		...
		if (isAccessTokenValid()) {
			deleteProduct(product.id, accessToken).then(() => {
				broadcastAlert("Product " + product.name + " deleted successfully.", "success");
				setBusy(false);
				setProduct(null);

...
const ProductPage = ({ categories, mode, headingText, buttonText, callbackFunction, jsonData, reFetchAllData }) => {
	...
	const { accessToken, isAccessTokenValid, logout } = useContext(AuthContext);
	...
	const initPageData = useCallback((type = mode, data = json, redirect = navigate, broadcast = broadcastAlert) => {
		if (type === null || (type === "MODIFY" && data === null)) {
			broadcast("Invalid access. Redirecting to home...", "warning");
			redirect("/home");
		}
	}, [mode, json, navigate, broadcastAlert]);
	useEffect(() => {
		initPageData();
	}, [initPageData]);
	...
	let validateData = () => {
		...
		if (valid) {
			if (isAccessTokenValid()) {
				...
				broadcastAlert("Session expired. Please login again!", "info");
				logout().then(() => {
					navigate("/login");
				});
			}
